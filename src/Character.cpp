...
void Character::MoveWithSprite(std::shared_ptr<MovableSprite>& sprite, glm::vec2 vector) {
	if (CollisionHandler::CheckCollision(Collider({ GetCollider().left, GetCollider().center.y }, { 0.1, GetCollider().size.y }), sprite->GetCollider())){
		if (vector.x < 0) rigidbody_.SetVelocity({ rigidbody_.GetVelocity().x + vector.x, rigidbody_.GetVelocity().y});
	}
	else if (CollisionHandler::CheckCollision(Collider({ GetCollider().right,  GetCollider().center.y }, { 0.1, GetCollider().size.y }), sprite->GetCollider())) {
		if (vector.x > 0) rigidbody_.SetVelocity({ rigidbody_.GetVelocity().x + vector.x,rigidbody_.GetVelocity().y });
	}
	else if (CollisionHandler::CheckCollision(Collider({ GetCollider().center.x, GetCollider().bottom }, { GetCollider().size.x, 0.1}), sprite->GetCollider())) {
		rigidbody_.SetVelocity({ rigidbody_.GetVelocity().x + vector.x, rigidbody_.GetVelocity().y + vector.y });
	}
}