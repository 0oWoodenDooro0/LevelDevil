...
Portal::Portal(std::vector<std::string> image_pahts, glm::vec2 position, AudioManager audio_manager) : audio_manager_(std::move(audio_manager)), goal_(position) {
    ...
    animator_.SetAnimationStates(
        { {"s1", std::make_unique<Util::Image>(image_pahts[0])},
...
void Portal::Update(const std::shared_ptr<Character>& character_) {
    ...
    if (current_state_ == State::s1 && draw_timer_<=0) {
        animator_.UpdateAnimationState("s2", set_drawable_function);
        current_state_ = State::s2;
        draw_timer_ = 100;
    }
    else if (current_state_ == State::s2 && draw_timer_ <= 0)
    {
        animator_.UpdateAnimationState("s1", set_drawable_function);
        current_state_ = State::s1;
        draw_timer_ = 300;
    }
