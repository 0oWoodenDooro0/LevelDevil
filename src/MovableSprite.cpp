...
void MovableSprite::Move(glm::vec2 target_position, float speed, const Character &character) {
    ...
    if (hypot(vector.x, vector.y) != 0) {
        ...
        if (CollisionHandler::CheckCollisionWithBorder(character.GetCollider(), GetCollider())) {
            character.GetRigidbody().SetVelocity({});
        }
